...
def create_cwp_auto(db: Session, cwp: schemas.CWPCreate):
    ...
    Nota: La prioridad ahora se maneja a nivel de CWA, no aquí.
    """
...
def update_cwp(db: Session, cwp_id: int, cwp_update: schemas.CWPUpdate):
    """
    Actualizar un CWP existente con campos opcionales
    """
    ...
    for index, row in df_data.iterrows():
        try:
            ...
            if not db_cwp:
                ...
                db_cwp = models.CWP(nombre=f"Paquete {cwp_code}", codigo=cwp_code, cwa_id=cache_cwa[cwa_code], disciplina_id=disciplina_default, estado="NO_INICIADO")
            ...
            if not db_pkg:
                db_pkg = models.Paquete(nombre=f"Paquete {pkg_code}", codigo=pkg_code, tipo=pkg_type, responsable="Importado", cwp_id=db_cwp.id)
            ...
            if not db.query(models.Item).filter(models.Item.paquete_id == db_pkg.id, models.Item.nombre == item_name).first():
                new_item = models.Item(nombre=item_name, descripcion=str(row.get('Descripcion', '')), tipo_entregable_id=tipo_id, paquete_id=db_pkg.id, estado="NO_INICIADO")
                db.add(new_item); stats["items_creados"] += 1

        except Exception as e: stats["errores"].append(f"Fila {index+2}: {str(e)}"); continue
...
def get_tipos_entregables_disponibles(db: Session, cwp_id: int):
    ...
    return db.query(models.TipoEntregable).filter((models.TipoEntregable.disciplina_id == db_cwp.disciplina_id) | (models.TipoEntregable.es_generico == True)).all()
