...
function AWPTableConsolidada({ plotPlanId, proyecto, filteredCWAId, onDataChange }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Auto-expandir todo al cargar (primera vez) si no hay filtro
  ...
  // ============================================================================
  // 6. LÓGICA DE FILTRADO Y SINCRONIZACIÓN VISUAL
  // ============================================================================

  // Filtramos la jerarquía para renderizar
  const cwasToRender = jerarquia?.cwas?.filter(cwa => {
    ...
    // 2. Filtro por Selección en el Plano (Si filteredCWAId existe, SOLO mostramos ese CWA)
    // Si no hay filtro visual (filteredCWAId es null), mostramos todo.
    const matchSelection = !filteredCWAId || cwa.id === filteredCWAId;
    
    return matchText && matchSelection;
  }).sort((a, b) => a.codigo.localeCompare(b.codigo));

  // Efecto para expandir automáticamente cuando se selecciona desde el plano
  useEffect(() => {
    if (filteredCWAId && jerarquia) {
      // Expandir el CWA seleccionado
      setExpandedCWAs(prev => new Set(prev).add(filteredCWAId));
      
      // Opcional: Expandir también sus hijos (CWPs) para ver el contenido inmediato
  ...
  // ============================================================================
  // 9. HANDLERS MODALES (CWP, Paquete, Items)
  // ============================================================================
  ...
  const handleDeleteCWP = async (id) => {
    ...
    try { 
      ...
      loadData(); 
    } catch(e) { alert("Error borrando CWP"); }
  };
  ...
  const saveBatch = async (pkg) => {
    ...
    try {
      ...
      const newP = {...pendingItems}; delete newP[pkg.id]; setPendingItems(newP); 
      loadData();
            ...
            <tr>
              ...
              <th className="p-3 min-w-[300px] sticky left-10 bg-hatch-gray z-30 border-r-2 border-b-2 border-hatch-gray-dark">
                <div className="flex flex-col gap-2">
                  ...
                  <input className="bg-white border-2 border-hatch-gray-dark rounded px-3 py-1 text-hatch-blue font-normal text-xs focus:border-hatch-orange outline-none placeholder-gray-400" placeholder="🔍 Filtrar por código..." onChange={e => setFilters({...filters, codigo: e.target.value})} />
                </div>
              </th>
              ...
              {customColumns.map(c => (<th key={c.id} className="p-3 border-l-2 border-b-2 border-hatch-gray-dark text-hatch-orange whitespace-nowrap min-w-[120px]">{c.nombre}</th>))}
                  ...
                  <tr className={`transition-colors ${filteredCWAId === cwa.id ? 'bg-yellow-50 border-l-4 border-yellow-400' : 'bg-white hover:bg-hatch-gray/30'}`}>
                    ...
                    <td className="p-3 font-bold text-hatch-blue sticky left-10 bg-inherit z-10 border-r-2 border-hatch-gray">
                      <div className="flex items-center gap-3">
                        <span className={`text-[10px] px-2 py-1 rounded-full font-semibold ${cwa.es_transversal ? 'bg-purple-100 border border-purple-300 text-purple-700' : 'bg-blue-100 border border-blue-300 text-blue-700'}`}>{cwa.es_transversal ? 'DWP' : 'CWA'}</span>
                    ...
                    <td className="p-3 text-center">
                      <select 
                        className={`bg-white text-xs font-bold border-2 rounded-lg px-2 py-1 cursor-pointer transition-all hover:border-hatch-orange focus:outline-none focus:ring-2 focus:ring-hatch-orange ${cwa.prioridad === 'CRITICA' ? 'border-red-400 text-red-600' : cwa.prioridad === 'ALTA' ? 'border-orange-400 text-orange-600' : 'border-hatch-gray text-gray-600'}`}
                    ...
                    <td colSpan={3 + customColumns.length} className="p-3 text-xs text-gray-500 italic">📍 {cwa.plot_plan_nombre}</td>
                      ...
                      <React.Fragment key={cwp.id}>
                        <tr className="bg-hatch-gray/20 hover:bg-hatch-gray/40 border-t border-hatch-gray transition-colors">
                          ...
                          <td className="p-3 pl-8 sticky left-10 bg-inherit z-10 border-r-2 border-hatch-gray">
                            <div className="flex items-center gap-3">
                              <button onClick={() => toggle(expandedCWPs, cwp.id, setExpandedCWPs)} className="text-hatch-blue hover:text-hatch-orange text-sm font-bold w-4 transition-colors">{isCwpExp ? '▼' : '▶'}</button>
                              <span className="text-green-600 font-mono text-xs bg-green-50 px-2 py-1 rounded border border-green-200">{cwp.codigo}</span>
                              <span className="text-hatch-blue font-medium text-sm truncate max-w-[200px]" title={cwp.nombre}>{cwp.nombre}</span>
                            </div>
                          </td>
                          <td className="p-3 text-center text-gray-400 text-xs">-</td>
                          <td className="p-3 text-center">
                            <input type="number" className="w-12 bg-white border-2 border-hatch-gray rounded text-center text-sm text-hatch-blue focus:border-hatch-orange outline-none hover:border-hatch-orange transition-colors" value={cwp.secuencia || 0} onChange={e => updateCWPField(cwp.id, 'secuencia', e.target.value)} />
                          </td>
                          <td className="p-3 text-center text-xs">
                            <div className="flex justify-center gap-2 items-center">
                              <input type="date" className="bg-white border-2 border-hatch-gray rounded w-28 text-hatch-blue text-[11px] px-2 py-1 outline-none" value={cwp.forecast_inicio?.split('T')[0] || ''} onChange={e => updateCWPField(cwp.id, 'forecast_inicio', e.target.value)} title="Inicio" />
                              <span className="text-gray-400 font-bold">➜</span>
                              <input type="date" className="bg-white border-2 border-hatch-gray rounded w-28 text-hatch-blue text-[11px] px-2 py-1 outline-none" value={cwp.forecast_fin?.split('T')[0] || ''} onChange={e => updateCWPField(cwp.id, 'forecast_fin', e.target.value)} title="Fin" />
                            </div>
                          </td>
                          <td className="p-3 text-center">
                            <div className="relative w-full bg-hatch-gray h-6 rounded-full overflow-hidden border border-hatch-gray-dark">
                              <div className="absolute top-0 left-0 h-full bg-gradient-to-r from-green-400 to-green-500 transition-all duration-300" style={{width: `${cwp.porcentaje_completitud}%`}}></div>
                              <span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-hatch-blue">{cwp.porcentaje_completitud}%</span>
                            </div>
                          </td>
                          {customColumns.map(c => (<td key={c.id} className="p-3 border-l-2 border-hatch-gray text-xs"><span className="bg-hatch-gray text-hatch-blue px-2 py-1 rounded border border-hatch-gray-dark">{cwp.metadata_json?.[c.nombre] || '-'}</span></td>))}
                          <td className="p-3 text-right">
                            <div className="flex justify-end gap-1 items-center">
                              <button onClick={() => openCWPModal(null, cwp)} className="text-gray-500 hover:text-hatch-orange text-sm p-1.5 rounded hover:bg-hatch-gray transition-colors">✏️</button>
                              <button onClick={() => handleDeleteCWP(cwp.id)} className="text-red-400 hover:text-red-600 text-sm p-1.5 rounded hover:bg-red-50 transition-colors">🗑️</button>
                              <div className="w-px h-6 bg-hatch-gray mx-1"></div>
                              <button onClick={() => openPkgModal(cwp, 'EWP')} className="text-[10px] bg-purple-100 text-purple-700 px-2 py-1 rounded hover:bg-purple-200 transition-colors border border-purple-300 font-semibold">+EWP</button>
                              <button onClick={() => openPkgModal(cwp, 'PWP')} className="text-[10px] bg-teal-100 text-teal-700 px-2 py-1 rounded hover:bg-teal-200 transition-colors border border-teal-300 font-semibold">+PWP</button>
                              <button onClick={() => openPkgModal(cwp, 'IWP')} className="text-[10px] bg-orange-100 text-orange-700 px-2 py-1 rounded hover:bg-orange-200 transition-colors border border-orange-300 font-semibold">+IWP</button>
                            </div>
                          </td>
                        </tr>
                              ...
                              <tr className="bg-white hover:bg-hatch-gray/10 text-xs group border-t border-hatch-gray/50 transition-colors">
                                ...
                                <td className="p-2 pl-16 sticky left-10 bg-inherit z-10 border-r-2 border-hatch-gray">
                                  <div className="flex items-center gap-2 text-gray-700">
                                    <button onClick={() => toggle(expandedPaquetes, pkg.id, setExpandedPaquetes)} className="hover:text-hatch-orange text-xs font-bold w-3 transition-colors">{isPkgExp ? '▼' : '▶'}</button>
                                    <span className={`text-[9px] border px-1.5 py-0.5 rounded font-semibold ${pkg.tipo === 'EWP' ? 'border-purple-300 text-purple-700 bg-purple-50' : pkg.tipo === 'PWP' ? 'border-teal-300 text-teal-700 bg-teal-50' : 'border-orange-300 text-orange-700 bg-orange-50'}`}>{pkg.tipo}</span>
                                ...
                                <>
                                  {pkg.items.map(item => (
                                    <tr key={item.id} className="bg-hatch-gray/5 text-[11px] hover:bg-hatch-gray/20 transition-colors border-t border-hatch-gray/30">
                                      ...
                                      <td className="p-2 pl-24 sticky left-10 bg-inherit z-10 border-r-2 border-hatch-gray/50 text-gray-500 truncate max-w-[250px]">
                                        {item.source_item_id ? <span className="text-blue-500 flex items-center gap-1 text-[10px]" title={`Origen: ${item.origen_info}`}>🔗 {item.origen_info}</span> : <span className="text-gray-400">ID: {item.id}</span>}
                                      </td>
                                      <td colSpan={2} className="p-2 text-hatch-blue pl-4">
                                        <div className="flex items-center gap-2">
                                          <span>{item.nombre}</span>
                                          <button onClick={() => openClassifyModal(item, cwp.id)} className="text-[9px] text-gray-500 hover:text-hatch-orange border border-hatch-gray px-1.5 py-0.5 rounded hover:bg-hatch-gray transition-colors font-medium">{item.tipo_entregable_codigo || "🏷️ Clasificar"}</button>
                                        </div>
                                      </td>
                                      <td className="p-2 text-center">
                                        <input type="date" className="bg-white border border-hatch-gray rounded w-24 text-gray-600 text-[10px] text-center px-1 py-0.5 hover:border-hatch-orange focus:border-hatch-orange outline-none transition-colors cursor-pointer" value={item.forecast_fin?.split('T')[0] || ''} onChange={e => updateItemForecast(item.id, e.target.value)} />
                                      </td>
                                  ...
                                  {pendingItems[pkg.id]?.map(t => (
                                    <tr key={t.id} className="bg-yellow-50 text-xs animate-pulse border-t border-yellow-200">
                                      ...
                                      <td className="p-2 pl-24 text-yellow-700 sticky left-10 border-r-2 border-yellow-200 font-semibold">⚡ Nuevo</td>
                                      <td colSpan={3} className="p-2">
                                        <input autoFocus className="w-full bg-white border-2 border-yellow-400 rounded text-hatch-blue outline-none px-2 py-1 placeholder-yellow-600 focus:border-hatch-orange transition-colors" value={t.nombre} onChange={e => changeBatch(pkg.id, t.id, e.target.value)} onKeyDown={e => {if(e.key === 'Enter') saveBatch(pkg)}} placeholder="Escribe el nombre del entregable..." />
                                      </td>
                                      <td colSpan={2 + customColumns.length}></td>
                                      <td className="p-2 text-right">
                                        <button onClick={() => setPendingItems({...pendingItems, [pkg.id]: pendingItems[pkg.id].filter(i => i.id !== t.id)})} className="text-red-500 hover:bg-red-50 px-2 py-1 rounded transition-colors">✕</button>
                                      </td>
                                    </tr>
                                  ))}
                                  ...
                                  {pendingItems[pkg.id]?.length > 0 && (
                                    <tr className="bg-yellow-100 border-t-2 border-yellow-400">
                                      <td colSpan="100%" className="text-center p-3">
                                        <button onClick={() => saveBatch(pkg)} className="bg-gradient-orange hover:shadow-lg text-white font-bold px-6 py-2 rounded-lg uppercase tracking-wider text-sm transition-all">💾 Guardar {pendingItems[pkg.id].length} Item(s)</button>
                                      </td>
                                    </tr>
                                  )}
                                </>
          ...
          <div className="bg-white w-[500px] p-6 rounded-2xl border-2 border-hatch-gray shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input className="w-full bg-white text-hatch-blue border-2 border-hatch-gray rounded-lg px-3 py-2 focus:border-hatch-orange outline-none transition-colors" placeholder="Ej: Paquete de Estructuras" value={formData.nombre} onChange={e => setFormData({...formData, nombre: e.target.value})} />
              </div>
              {!isEditingCWP && (
                <div>
                  <label className="block text-xs text-gray-600 mb-1 font-semibold">Disciplina</label>
                  <select className="w-full bg-white text-hatch-blue border-2 border-hatch-gray rounded-lg px-3 py-2 focus:border-hatch-orange outline-none transition-colors" value={formData.disciplina_id} onChange={e => setFormData({...formData, disciplina_id: e.target.value})}>
              ...
              {customColumns.length > 0 && (
                <div className="border-t-2 border-hatch-gray pt-4">
                  <p className="text-hatch-orange text-xs font-bold mb-3 uppercase tracking-wider">🏷️ Metadatos Personalizados</p>
                  {customColumns.map(c => (
                    <div key={c.id} className="mb-3">
                      <label className="block text-xs text-gray-600 mb-1 font-semibold">{c.nombre}</label>
                      {c.tipo_dato === 'SELECCION' ? (
                        <select className="w-full bg-white text-hatch-blue border-2 border-hatch-gray rounded-lg px-2 py-1.5 text-sm focus:border-hatch-orange outline-none transition-colors" value={formData.metadata?.[c.nombre] || ''} onChange={e => setFormData({...formData, metadata: {...formData.metadata, [c.nombre]: e.target.value}})}><option value="">- Seleccionar -</option>{c.opciones_json?.map(o => <option key={o} value={o}>{o}</option>)}</select>
                      ) : (
                        <input className="w-full bg-white text-hatch-blue border-2 border-hatch-gray rounded-lg px-2 py-1.5 text-sm focus:border-hatch-orange outline-none transition-colors" value={formData.metadata?.[c.nombre] || ''} onChange={e => setFormData({...formData, metadata: {...formData.metadata, [c.nombre]: e.target.value}})} />
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t-2 border-hatch-gray">
              <button onClick={() => setModals({...modals, cwp: false})} className="text-gray-600 hover:text-hatch-blue px-4 py-2 transition-colors font-medium">Cancelar</button>
              <button onClick={handleSaveCWP} className="bg-gradient-orange hover:shadow-lg text-white px-6 py-2 rounded-lg font-bold transition-all">{isEditingCWP ? 'Guardar Cambios' : 'Crear CWP'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modals.pkg && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl w-96 border-2 border-hatch-gray shadow-2xl">
            ...
            <input className="w-full mb-4 bg-white text-hatch-blue border-2 border-hatch-gray rounded-lg px-3 py-2 focus:border-hatch-orange outline-none transition-colors" placeholder="Nombre del Paquete" value={formData.nombre} onChange={e => setFormData({...formData, nombre: e.target.value})} />
            <div className="flex justify-end gap-3">
              <button onClick={() => setModals({...modals, pkg: false})} className="text-gray-600 hover:text-hatch-blue px-4 py-2 transition-colors font-medium">Cancelar</button>
              <button onClick={handleSavePkg} className="bg-gradient-orange hover:shadow-lg text-white px-6 py-2 rounded-lg font-bold transition-all">Crear</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white w-[650px] p-6 rounded-2xl border-2 border-hatch-gray h-[85vh] flex flex-col shadow-2xl">
            ...
            <div className="flex gap-2 mb-3">
              <button onClick={() => setLinkFilter("ALL")} className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${linkFilter === "ALL" ? 'bg-gradient-orange text-white shadow-lg' : 'bg-hatch-gray text-hatch-blue'}`}>Todo el Proyecto</button>
              <button onClick={() => setLinkFilter("TRANSVERSAL")} className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${linkFilter === "TRANSVERSAL" ? 'bg-gradient-orange text-white shadow-lg' : 'bg-hatch-gray text-hatch-blue'}`}>Solo Transversales</button>
            </div>
            <div className="flex-1 overflow-y-auto bg-hatch-gray/20 p-3 rounded-lg border-2 border-hatch-gray">
              {transversalItems.filter(i => linkFilter === "ALL" || i.es_transversal).map(item => (
                <div key={item.id} className={`flex items-center p-3 mb-2 rounded-lg cursor-pointer border-2 transition-all ${selectedLinkItems.has(item.id) ? 'border-hatch-orange bg-orange-50' : 'border-hatch-gray bg-white'}`} onClick={() => toggle(selectedLinkItems, item.id, setSelectedLinkItems)}>
                  ...
                  <div><p className="text-hatch-blue text-sm font-medium">{item.nombre}</p><p className="text-gray-500 text-xs">{item.cwa} ➝ {item.paquete}</p></div>
                </div>
              ))}
            </div>
            <div className="mt-4 flex justify-end gap-3 pt-4 border-t-2 border-hatch-gray">
              <button onClick={() => setModals({...modals, link: false})} className="text-gray-600 hover:text-hatch-blue px-4 py-2 transition-colors font-medium">Cancelar</button>
              <button onClick={handleLinkItems} className="bg-gradient-orange hover:shadow-lg text-white px-6 py-2 rounded-lg font-bold transition-all">Vincular ({selectedLinkItems.size})</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modals.editItem && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl w-96 border-2 border-hatch-gray shadow-2xl">
            ...
            <select className="w-full bg-white text-hatch-blue p-2 rounded-lg mb-6 border-2 border-hatch-gray focus:border-hatch-orange outline-none transition-colors" value={editingItem?.tipo_entregable_id || ''} onChange={e => setEditingItem({...editingItem, tipo_entregable_id: e.target.value})}>
              <option value="">- Sin Tipo -</option>
              {itemTipos.map(t => <option key={t.id} value={t.id}>{t.codigo} - {t.nombre}</option>)}
            </select>
            <div className="flex justify-end gap-3">
              <button onClick={() => setModals({...modals, editItem: false})} className="text-gray-600 hover:text-hatch-blue px-4 py-2 transition-colors font-medium">Cancelar</button>
              <button onClick={handleSaveClassify} className="bg-gradient-orange hover:shadow-lg text-white px-6 py-2 rounded-lg font-bold transition-all">Guardar</button>
            </div>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleImport}>
              <label className="block mb-4 cursor-pointer">
                <div className="w-full border-2 border-dashed border-hatch-gray rounded-lg p-8 text-center hover:border-hatch-orange transition-colors bg-hatch-gray/20">
                  {importFile ? <div className="flex flex-col items-center gap-2"><span className="text-4xl">📄</span><span className="text-hatch-blue font-medium text-sm">{importFile.name}</span></div> : <div className="flex flex-col items-center gap-2"><span className="text-4xl text-gray-400">📁</span><span className="text-gray-600 text-sm">Haz click para seleccionar</span></div>}
                </div>
                <input type="file" accept=".csv,.xlsx" onChange={e => setImportFile(e.target.files[0])} className="hidden" />
              </label>
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setModals({...modals, import: false})} className="text-gray-600 hover:text-hatch-blue px-4 py-2 transition-colors font-medium">Cancelar</button>
                <button type="submit" disabled={importing || !importFile} className="bg-gradient-orange hover:shadow-lg disabled:opacity-50 text-white px-6 py-2 rounded-lg font-bold transition-all disabled:cursor-not-allowed">{importing ? 'Procesando...' : 'Importar'}</button>
              </div>
            </form>
          </div>
        </div>
